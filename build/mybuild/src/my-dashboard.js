...
if(-1!==this.tabindex){this._previousTabIndex=this.tabindex}this.tabindex=tabindex=void 0}if(window.ShadyDOM){this.setProperties({tabIndex:tabindex,tabindex:tabindex})}}/**
    ...
    * @polymerMixin
    */_exports.ControlStateMixin=ControlStateMixin;var vaadinControlStateMixin={ControlStateMixin:ControlStateMixin};_exports.$vaadinControlStateMixin=vaadinControlStateMixin;const DEV_MODE_CODE_REGEXP=/\/\*\*\s+vaadin-dev-mode:start([\s\S]*)vaadin-dev-mode:end\s+\*\*\//i;function isMinified(){function test(){/** vaadin-dev-mode:start
    return false;
...
if(this.is){window.Vaadin.registrations.push(this);if(window.Vaadin.developmentModeCallback){statsJob=_shared_bundle_.Debouncer.debounce(statsJob,_shared_bundle_.idlePeriod,()=>{window.Vaadin.developmentModeCallback["vaadin-usage-statistics"]()});(0,_shared_bundle_.enqueueDebouncer)(statsJob)}}}ready(){super.ready();if(null===document.doctype){console.warn("Vaadin components require the \"standards mode\" declaration. Please add <!DOCTYPE html> to the HTML document.")}}};_exports.ElementMixin=ElementMixin;var vaadinElementMixin={ElementMixin:ElementMixin};_exports.$vaadinElementMixin=vaadinElementMixin;class CheckboxElement extends ElementMixin(ControlStateMixin(ThemableMixin((0,_shared_bundle_.GestureEventListeners)(_shared_bundle_.PolymerElement)))){static get template(){return _shared_bundle_.html$1`
    <style>
      :host {
...
if("a"===e.target.localName){return!1}return!0}_handleClick(e){if(this.__interactionsAllowed(e)){if(!this.indeterminate){if(e.composedPath()[0]!==this._nativeCheckbox){e.preventDefault();this._toggleChecked()}}else{/*
     ...
     *
     * @event change
     */}_exports.CheckboxElement=CheckboxElement;customElements.define(CheckboxElement.is,CheckboxElement);var vaadinCheckbox={CheckboxElement:CheckboxElement};_exports.$vaadinCheckbox=vaadinCheckbox;class Lumo extends HTMLElement{static get version(){return"1.4.2"}}_exports.Lumo=Lumo;customElements.define("vaadin-lumo-styles",Lumo);var version={Lumo:Lumo};_exports.$version=version;const $_documentContainer=document.createElement("template");$_documentContainer.innerHTML=`<custom-style>
  <style>
    html {
      /* Base (background) */
      --lumo-base-color: #FFF;
...
</custom-style>`;document.head.appendChild($_documentContainer$1.content);const $_documentContainer$2=_shared_bundle_.html$1`<dom-module id="lumo-checkbox" theme-for="vaadin-checkbox">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$2.content);var IOS=navigator.userAgent.match(/iP(?:hone|ad;(?: U;)? CPU) OS (\d+)/),IOS_TOUCH_SCROLLING=IOS&&8<=IOS[1],DEFAULT_PHYSICAL_COUNT=3,ANIMATION_FRAME=_shared_bundle_.animationFrame,IDLE_TIME=_shared_bundle_.idlePeriod,MICRO_TASK=_shared_bundle_.microTask;const PolymerIronList=(0,_shared_bundle_.Class)({behaviors:[_shared_bundle_.IronResizableBehavior,_shared_bundle_.IronScrollTargetBehavior],/**
...
_shared_bundle_.flush?(0,_shared_bundle_.flush)():(0,_shared_bundle_.flush$1)();var newPhysicalSize=0,oldPhysicalSize=0,prevAvgCount=this._physicalAverageCount,prevPhysicalAvg=this._physicalAverage;this._iterateItems(function(pidx,vidx){oldPhysicalSize+=this._physicalSizes[pidx];this._physicalSizes[pidx]=this._physicalItems[pidx].offsetHeight;newPhysicalSize+=this._physicalSizes[pidx];this._physicalAverageCount+=this._physicalSizes[pidx]?1:0},itemSet);if(this.grid){this._updateGridMetrics();this._physicalSize=Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight}else{oldPhysicalSize=1===this._itemsPerRow?oldPhysicalSize:Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight;this._physicalSize=this._physicalSize+newPhysicalSize-oldPhysicalSize;this._itemsPerRow=1}// Update the average if it measured something.
...
if(forceUpdate||Math.abs(this._estScrollHeight-this._scrollHeight)>=this._viewportHeight){this.$.items.style.height=this._estScrollHeight+"px";this._scrollHeight=this._estScrollHeight}},/**
   ...
   */scrollToIndex:function(idx){if("number"!==typeof idx||0>idx||idx>this.items.length-1){return}_shared_bundle_.flush?(0,_shared_bundle_.flush)():(0,_shared_bundle_.flush$1)();// Items should have been rendered prior scrolling to an index.
...
this._itemsPerRow=this._itemsPerRow||1;return this.grid?Math.ceil(idx/this._itemsPerRow)*this._itemsPerRow:idx},_isIndexRendered:function(idx){return idx>=this._virtualStart&&idx<=this._virtualEnd},_getPhysicalIndex:function(vidx){return(this._physicalStart+(vidx-this._virtualStart))%this._physicalCount},_clamp:function(v,min,max){return Math.min(max,Math.max(min,v))},_debounce:function(name,cb,asyncModule){this._debouncers=this._debouncers||{};this._debouncers[name]=_shared_bundle_.Debouncer.debounce(this._debouncers[name],asyncModule,cb.bind(this));(0,_shared_bundle_.enqueueDebouncer)(this._debouncers[name])}});_exports.PolymerIronList=PolymerIronList;var ironList={PolymerIronList:PolymerIronList};/**
   @license
   Copyright (c) 2017 Vaadin Ltd.
   This program is available under Apache License Version 2.0, available at https://vaadin.com/license/
   */ /**
...
if(_shared_bundle_.Base.get(parentProperty,items[0])===void 0){console.warn(`Path "${path}" used for ${action} does not exist in all of the items, ${action} is disabled.`);result=!1}}return result}_multiSort(a,b){return this._sorters.map(sort=>{if("asc"===sort.direction){return this._compare(_shared_bundle_.Base.get(sort.path,a),_shared_bundle_.Base.get(sort.path,b))}else if("desc"===sort.direction){return this._compare(_shared_bundle_.Base.get(sort.path,b),_shared_bundle_.Base.get(sort.path,a))}return 0}).reduce((p,n)=>{return p?p:n},0)}_normalizeEmptyValue(value){if(0<=[void 0,null].indexOf(value)){return""}else if(isNaN(value)){return value.toString()}else{return value}}_compare(a,b){a=this._normalizeEmptyValue(a);b=this._normalizeEmptyValue(b);if(a<b){return-1}if(a>b){return 1}return 0}_filter(items){return items.filter((item,index)=>{return 0===this._filters.filter(filter=>{const value=this._normalizeEmptyValue(_shared_bundle_.Base.get(filter.path,item)),filterValueLowercase=this._normalizeEmptyValue(filter.value).toString().toLowerCase();return-1===value.toString().toLowerCase().indexOf(filterValueLowercase)}).length})}};_exports.ArrayDataProviderMixin=ArrayDataProviderMixin;var vaadinGridArrayDataProviderMixin={ArrayDataProviderMixin:ArrayDataProviderMixin};_exports.$vaadinGridArrayDataProviderMixin=vaadinGridArrayDataProviderMixin;const ColumnReorderingMixin=superClass=>class ColumnReorderingMixin extends(0,_shared_bundle_.GestureEventListeners)(superClass){static get properties(){return{/**
       * Set to true to allow column reordering.
       */columnReorderingAllowed:{type:Boolean,value:!1},_orderBaseScope:{type:Number,value:1e7}}}static get observers(){return["_updateOrders(_columnTree, _columnTree.*)"]}ready(){super.ready();(0,_shared_bundle_.addListener)(this,"track",this._onTrackEvent);this._reorderGhost=this.shadowRoot.querySelector("[part=\"reorder-ghost\"]");this.addEventListener("touchstart",this._onTouchStart.bind(this));this.addEventListener("touchmove",this._onTouchMove.bind(this));this.addEventListener("touchend",this._onTouchEnd.bind(this));this.addEventListener("contextmenu",this._onContextMenu.bind(this))}_onContextMenu(e){if(this.hasAttribute("reordering")){e.preventDefault()}}_onTouchStart(e){// Touch event, delay activation by 100ms
...
if(this._grid&&this._grid.__forceReflow){this._grid.__forceReflow()}}_frozenChanged(frozen,headerCell,footerCell,cells){if(this.parentElement&&this.parentElement._columnPropChanged){this.parentElement._columnPropChanged("frozen",frozen)}this._allCells.forEach(cell=>this._toggleAttribute("frozen",frozen,cell));this._grid&&this._grid._frozenCellsChanged&&this._grid._frozenCellsChanged()}_lastFrozenChanged(lastFrozen){this._allCells.forEach(cell=>this._toggleAttribute("last-frozen",lastFrozen,cell));if(this.parentElement&&this.parentElement._columnPropChanged){this.parentElement._lastFrozen=lastFrozen}}_pathOrHeaderChanged(path,header,headerCell,footerCell,cells,renderer,headerRenderer,bodyTemplate,headerTemplate){const hasHeaderText=header!==void 0;if(!headerRenderer&&!headerTemplate&&hasHeaderText&&headerCell){this.__setTextContent(headerCell._content,header)}if(path&&cells.value){if(!renderer&&!bodyTemplate){const pathRenderer=(root,owner,{item})=>this.__setTextContent(root,this.get(path,item));this.__setColumnTemplateOrRenderer(void 0,pathRenderer,cells.value)}if(!headerRenderer&&!headerTemplate&&!hasHeaderText&&headerCell&&null!==header){this.__setTextContent(headerCell._content,this._generateHeader(path))}}if(headerCell){this._grid.__updateHeaderFooterRowVisibility(headerCell.parentElement)}}__setTextContent(node,textContent){node.textContent!==textContent&&(node.textContent=textContent)}_generateHeader(path){return path.substr(path.lastIndexOf(".")+1).replace(/([A-Z])/g,"-$1").toLowerCase().replace(/-/g," ").replace(/^./,match=>match.toUpperCase())}_toggleAttribute(name,bool,node){if(node.hasAttribute(name)===!bool){if(bool){node.setAttribute(name,"")}else{node.removeAttribute(name)}}}_reorderStatusChanged(reorderStatus,headerCell,footerCell,cells){this._allCells.forEach(cell=>cell.setAttribute("reorder-status",reorderStatus))}_resizableChanged(resizable,headerCell){if(resizable===void 0||headerCell===void 0){return}if(headerCell){[headerCell].concat(this._emptyCells).forEach(cell=>{if(cell){const existingHandle=cell.querySelector("[part~=\"resize-handle\"]");if(existingHandle){cell.removeChild(existingHandle)}if(resizable){const handle=document.createElement("div");handle.setAttribute("part","resize-handle");cell.appendChild(handle)}}})}}_textAlignChanged(textAlign,_cells,_headerCell,_footerCell){if(textAlign===void 0){return}if(-1===["start","end","center"].indexOf(textAlign)){console.warn("textAlign can only be set as \"start\", \"end\" or \"center\"");return}let textAlignFallback;if("ltr"===getComputedStyle(this._grid).direction){if("start"===textAlign){textAlignFallback="left"}else if("end"===textAlign){textAlignFallback="right"}}else{if("start"===textAlign){textAlignFallback="right"}else if("end"===textAlign){textAlignFallback="left"}}this._allCells.forEach(cell=>{cell._content.style.textAlign=textAlign;if(getComputedStyle(cell._content).textAlign!==textAlign){cell._content.style.textAlign=textAlignFallback}})}_hiddenChanged(hidden,headerCell,footerCell,cells){if(this.parentElement&&this.parentElement._columnPropChanged){this.parentElement._columnPropChanged("hidden",hidden)}this._allCells.forEach(cell=>this._toggleAttribute("hidden",hidden,cell));if(!!hidden!==!!this._previousHidden&&this._grid){this._grid._updateLastFrozen&&this._grid._updateLastFrozen();this._grid.notifyResize&&this._grid.notifyResize();this._grid._resetKeyboardNavigation&&this._grid._resetKeyboardNavigation()}this._previousHidden=hidden}};/**
    ...
    */_exports.ColumnBaseMixin=ColumnBaseMixin;class GridColumnElement extends ColumnBaseMixin(_shared_bundle_.PolymerElement){static get is(){return"vaadin-grid-column"}static get properties(){return{/**
...
this._scrollToIndex(this._firstVisibleIndex)}this._ensureFirstPageLoaded();this._debouncerCheckSize=_shared_bundle_.Debouncer.debounce(this._debouncerCheckSize,_shared_bundle_.timeOut.after(2e3),this._checkSize.bind(this));this._scrollHandler()}_ensureFirstPageLoaded(){if(!this._hasData){// load data before adding rows to make sure they have content when
...
this._dropLocation=DropLocation.EMPTY}else if(!row||row.parentNode!==this.$.items){// The dragover didn't occur on a body row, default to "empty"...
this._dropLocation=DropLocation.EMPTY;if(this.dropMode===DropMode.BETWEEN||this.dropMode===DropMode.ON_TOP_OR_BETWEEN){// ...unless if the drop mode allows setting the last row as the drag over item
row=Array.from(this.$.items.children).filter(row=>!row.hidden).pop();this._dropLocation=DropLocation.BELOW}}else{// The dragover occurred on a body row, determine the drop location from coordinates
...
this._scrollingFrame=requestAnimationFrame(()=>this._toggleAttribute("scrolling",!0,this.$.scroller))}this._debounceScrolling=_shared_bundle_.Debouncer.debounce(this._debounceScrolling,_shared_bundle_.timeOut.after(this._timeouts.SCROLLING),()=>{cancelAnimationFrame(this._scrollingFrame);delete this._scrollingFrame;this._toggleAttribute("scrolling",!1,this.$.scroller);if(!this.$.outerscroller.outerScrolling){this._reorderRows()}});if(!this._scrollPeriodFrame){this._scrollPeriodFrame=requestAnimationFrame(()=>this._toggleAttribute("scroll-period",!0,this.$.scroller))}this._debounceScrollPeriod=_shared_bundle_.Debouncer.debounce(this._debounceScrollPeriod,_shared_bundle_.timeOut.after(this._timeouts.SCROLL_PERIOD),()=>{cancelAnimationFrame(this._scrollPeriodFrame);delete this._scrollPeriodFrame;this._toggleAttribute("scroll-period",!1,this.$.scroller)})}_afterScroll(){this._translateStationaryElements();if(!this.hasAttribute("reordering")){this._scheduleScrolling()}const os=this.$.outerscroller;if(!this._ios&&(this._wheelScrolling||os.passthrough)){os.syncOuterScroller()}if(this._ios){// Enable vertical rubberband feedback
...
let overflow="";const table=this.$.table;if(table.scrollTop<table.scrollHeight-table.clientHeight){overflow+=" bottom"}if(0<table.scrollTop){overflow+=" top"}if(table.scrollLeft<table.scrollWidth-table.clientWidth){overflow+=" right"}if(0<table.scrollLeft){overflow+=" left"}this._debounceOverflow=_shared_bundle_.Debouncer.debounce(this._debounceOverflow,_shared_bundle_.animationFrame,()=>{const value=overflow.trim();if(0<value.length&&this.getAttribute("overflow")!==value){this.setAttribute("overflow",value)}else if(0==value.length&&this.hasAttribute("overflow")){this.removeAttribute("overflow")}})}// correct order needed for preserving correct tab order between cell contents.
...
const content=this.querySelector("[slot]");if(content){const slot=content.getAttribute("slot");content.setAttribute("slot","foo-bar");content.setAttribute("slot",slot)}this._updateHeaderFooterMetrics();(0,_shared_bundle_.afterNextRender)(this,()=>this.notifyResize());return physicalItems}/**
...
VaadinGridStyles.appendChild(_shared_bundle_.html$1`
  <style>
...
this._update()}}/**
     ...
     */notifyResize(){super.notifyResize()}_heightByRowsChanged(value,oldValue){if(value||oldValue){this.notifyResize()}}__forceReflow(){this._debouncerForceReflow=_shared_bundle_.Debouncer.debounce(this._debouncerForceReflow,_shared_bundle_.animationFrame,()=>{this.$.scroller.style.overflow="hidden";setTimeout(()=>this.$.scroller.style.overflow="")})}}/**
...
</dom-module>`;document.head.appendChild($_documentContainer$6.content);const $_documentContainer$7=_shared_bundle_.html$1`<dom-module id="lumo-grid" theme-for="vaadin-grid">
  <template>
    <style>
      :host {
        font-family: var(--lumo-font-family);
...
</dom-module>`;document.head.appendChild($_documentContainer$7.content);class DashboardElement extends _shared_bundle_.PolymerElement{static get properties(){return{grid:{type:Array,value:{}},headergrid:{type:String,value:"Advance Search"},filterGrid:{type:Array,value:{}},isFoundData:{type:Boolean,value:!1},petname:{type:String,value:""},age:{type:Number,value:""},place:{type:String,value:""}}}static get template(){return _shared_bundle_.html`
 ...
 --paper-toast-background-color: red;
    ...
    #logintoast {
      ...
      <div class="adsearchSection">
<paper-button-group attr-for-selected="name" selected="c">
      <paper-button name="o">Items</paper-button>
      <paper-button name="c" style="float: right;"  on-click="_openAdvanceSearchModel">[[headergrid]]</paper-button>
      </paper-button-group>
</div>
      <vaadin-grid theme="row-dividers" column-reordering-allowed multi-sort items={{grid}}   >
      <vaadin-grid-column width="9em" path="productid">#</vaadin-grid-column>
      <vaadin-grid-column width="9em" path="itemname"></vaadin-grid-column>
      <vaadin-grid-column width="9em" path="price"></vaadin-grid-column>
      <vaadin-grid-column width="9em" path="quantity"></vaadin-grid-column>
      <vaadin-grid-column width="9em" path="description">
            <template>
                <paper-button raised class="custom indigo" on-click="_showDetails()">Details</paper-button>
            </template>
      </vaadin-grid-column>
      <vaadin-grid-column width="9em" path="buy">
          <template>
                <paper-button raised class="custom green"  on-click="_purchaseItem">Action</paper-button>
          </template>
      </vaadin-grid-column>
      </vaadin-grid>
           <!-- Show Details model start-->
          <paper-dialog id="detailsModal"  style=" outline: none;
          z-index: 103;
          position: fixed;
          top: 82.7031px;
          left: 332px;
          ...
          max-width: 700px;">
              <p>
                ...
                <div class="buttons">
                <paper-button  class="custom pink" dialog-confirm autofocus>Back</paper-button>

                <div class="buttons">
              
                <paper-button  raised class="custom indigo" dialog-confirm autofocus  on-click="_purchaseItem">Place Order</paper-button>
              </div>
          </paper-dialog>
      <!-- Show Details model End-->
    </div>

<!--Advance Search Start---->
    <paper-dialog id="searchModel">
                    <h2> Search Items:</h2>
                    <paper-dialog-scrollable>
                  <p> 
          <iron-form id="advanceSearch">
              <form> 
                <div>
                    <paper-input  value="{{itemname}}" always-float-label label="Item Name" error-message="Enter item name" auto-validate required></paper-input></paper-input>
                    <paper-input type="number"   min="1"
                    max="100"
                    step="1" value="{{quantity}}" always-float-label label="Quantity" error-message="Please enter quantity" auto-validate required></paper-input></paper-input>
                    <paper-input value="{{price}}" always-float-label label="Price" error-message="Please Enter price" auto-validate required></paper-input></paper-input>
                   
                </div>
                      <paper-button class="custom indigo" on-click="_advanceSearch">Search</paper-button>
                      <paper-button toggles class="custom indigo" class="green" on-click="_resetAdSearchForm">Reset Search</paper-button>
              </form>
          
                <template is="dom-if" if="{{isFoundData}}">
                        <vaadin-grid theme="row-dividers" on-click="_showDetails" column-reordering-allowed multi-sort items={{filterGrid}}>
                        <vaadin-grid-column width="9em" path="itemname"></vaadin-grid-column>
                        <vaadin-grid-column width="9em" path="price"></vaadin-grid-column>
                        <vaadin-grid-column width="9em" path="quantity"></vaadin-grid-column>
                        </vaadin-grid>
...
<iron-ajax
      id="filterlistAjax"
      on-response="_filterHandleResponse"
      on-error ="_handleError"
      debounce-duration="300">
  </iron-ajax>


<!--Div for default page-->
<iron-ajax
    id="listAjax"
    ...
    `}connectedCallback(){super.connectedCallback();let ajaxRef=this.$.listAjax;ajaxRef.method="get";ajaxRef.body="";ajaxRef.url="http://localhost:3000/items";ajaxRef.contentType="application/json";ajaxRef.generateRequest()}_findList(){const isValidate=this.$.signUpForm.validate()}_handleResponse(event){const response=event.detail.response;this.grid=event.detail.response}_handleError(error){alert("_handleError")}_showDetails(){this.$.detailsModal.open()}_purchaseItem(){window.history.pushState({},null,"/placeorder");window.dispatchEvent(new CustomEvent("location-changed"))}_openAdvanceSearchModel(){this.$.searchModel.open()}// itemname,quantity,price
_filterHandleResponse(event){const response=event.detail.response,passed=response.filter((element,index,array)=>{const filterCritaria={itemname:this.itemname?this.itemname:null,quantity:this.quantity?this.quantity:null,price:this.price?this.price:null};return element.itemname==this.itemname&&element.quantity==this.quantity});if(0<response.length){this.filterGrid=response;this.isFoundData=!0;response=[]}else{alert(`Sorry, We could not found any matching result`)}}_advanceSearch(){if(this.itemname||this.quantity||this.price){this.$.advanceSearch.reset();let ajaxRef=this.$.filterlistAjax;ajaxRef.method="get";ajaxRef.body="";ajaxRef.url="http://localhost:3000/items";ajaxRef.contentType="application/json";ajaxRef.generateRequest()}}_resetAdSearchForm(){this.$.advanceSearch.reset()}}customElements.define("dashboard-element",DashboardElement)});